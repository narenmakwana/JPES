  ...
  function controller($stateParams, OptionsService, FormConfigService, ComputedNameService, $timeout) {
      ...
      vm.submitForm = function(type, data){
        if(angular.isArray(vm.config[type])){
          ...
          generateComputedNames(vm.config[type][vm.config[type].length-1], FormConfigService.getComputedNameConfig(type));
        }
        else {
          vm.config[type] = data;
      ...
      vm.gotoForm = function(type){
        ...
        vm.form.data = angular.isArray(vm.config[type])? vm.config[type][vm.config[type].length-1] : {};
        $timeout(function(){
      ...
      var generateComputedNames = function(config, data){
        if(angular.isArray(data)){
          angular.forEach(data, function(item){
            var result = ComputedNameService.getComputedName(item.type), dest = item.alias?item.alias:item.type;
            config[dest] = result;
          })
        }
      };
